...
import { AddReportTitlesComponent } from './add-report-titles/add-report-titles.component';
...
@Component({
  ...
  styleUrls: ['./report-titles.component.scss']
})
export class ReportTitlesComponent implements OnInit {
  ...
  getGeneralInterfaces(){
    ...
    this.controlService.getGeneralInterface('OBJECTREPORT').subscribe({
      next:(res)=>{
  ...
  addGeneralInterface(){
    ...
    let modalRef = this.modalService.open(AddReportTitlesComponent,  {size:'lg',backdrop:'static'})
