...
import { AddBillInterfaceComponent } from './add-bill-interface/add-bill-interface.component';
...
@Component({
  selector: 'app-bill-interface',
  templateUrl: './bill-interface.component.html',
  styleUrls: ['./bill-interface.component.scss']
})
export class BillInterfaceComponent implements OnInit {
  ...
  getGeneralInterfaces(){
    ...
    this.controlService.getGeneralInterface('BILLINTERFACE').subscribe({
      next:(res)=>{
  ...
  addGeneralInterface(){
    ...
    let modalRef = this.modalService.open(AddBillInterfaceComponent,  {size:'lg',backdrop:'static'})
