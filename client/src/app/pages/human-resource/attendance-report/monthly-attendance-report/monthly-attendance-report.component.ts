...
export class MonthlyAttendanceReportComponent implements OnInit {
  ...
  generateReport() {
    ...
    this.hrmService.getMonthlyReport(dateStr, "PDF").subscribe({
  ...
  exportAsExcel() {
    const dateStr = this.attendanceDate.toISOString().split('T')[0];
    this.hrmService.getMonthlyReport(dateStr, "EXCELOPENXML").subscribe({
      next: (response: ArrayBuffer) => {
        ...
        saveAs(excelBlob, `MonthlyAttendance_${this.formatDate(this.attendanceDate)}.xlsx`);
      },
      error: (err) => {
        console.error('Error generating report:', err);
      }
