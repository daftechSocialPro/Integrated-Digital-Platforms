...
import { InventorySettlementReport } from 'src/app/model/Inventory/BalanceReportDto';
...
@Component({
  selector: 'app-settelment-report',
  templateUrl: './settelment-report.component.html',
  styleUrls: ['./settelment-report.component.css']
})
export class SettelmentReportComponent {

  @ViewChild('excelTable', { static: false }) excelTable!: ElementRef;
  @ViewChild('excelTable2', { static: false }) excelTable2!: ElementRef;

  printing: boolean = false;
  settelmentReportList!: InventorySettlementReport[];
  cols!: Column[];
  ...
  generateReport() {
    ...
    this.inventoryService.getSettelementReport(this.fromDate, this.toDate).subscribe({
      next: (res) => {
        this.settelmentReportList = res;
      },
    });
  }
  exportPdf() {
  ...
  exportPdf2() {
    ...
    if (cardElement) {
      const membername = `Inventory Settelment Report From ${this.fromDate} To ${this.toDate}.pdf`;
  ...
  exportAsExcel() {
    ...
    link.download = `Inventory Settelment Report From ${this.fromDate} To ${this.toDate}.xls`;
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      const worksheet = xlsx.utils.json_to_sheet(this.settelmentReportList);
