...
import { BalanceTempData } from 'src/app/model/Inventory/BalanceReportDto';
...
export class BalanceReportComponent implements OnInit {
  ...
  balanceReportList!: BalanceTempData[];
  cols!: Column[];
  ...
  generateReport() {
    ...
    this.inventoryService.getBalanceReport().subscribe({
      next: (res) => {
        this.balanceReportList = res;
      },
    });
  }
  exportPdf() {
  ...
  exportPdf2() {
    ...
    if (cardElement) {
      const membername = `$BalanceReport.pdf`;
  ...
  exportAsExcel() {
    ...
    link.download = `$balanceReport.xls`;
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      const worksheet = xlsx.utils.json_to_sheet(this.balanceReportList);
